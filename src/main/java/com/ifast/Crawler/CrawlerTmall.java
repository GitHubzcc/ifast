...
public class CrawlerTmall {
    ...
    public static void getArticleListFromUrl(String listUrl) {
        ...
        for (Element element : elements) {
            ...
            if (relHref.startsWith("http://") && relHref.contains("baidu")) {
                {
...
//			System.out.println(linkHref);
    ...
    public static void getArticleFromUrl(String detailurl) {
        try {
            ...
            Element elementTitle = document.getElementsByClass("s_menu_mine").first();//标题。 这边根据class的内容来过滤
            System.out.println(elementTitle.text()+"------elementTitle.text-----");
            String filename = elementTitle.text().replaceAll("/", "或");
            Element elementContent = document.getElementsByClass("mine-text").first();//内容。
